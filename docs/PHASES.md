...
**Existing files untouched:** `ingest_and_search.py`, `agent_graph.py`, `eval_pipeline.py`, root `requirements.txt`.

> **Phase 11 update:** These legacy files were moved to `legacy/` during Phase 11 cleanup. The modern backend has no dependency on them.
...
> **Phase 11 update:** Moved to `legacy/`. See Phase 2 note above.
